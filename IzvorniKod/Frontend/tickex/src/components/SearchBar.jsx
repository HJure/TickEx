...
const SearchBar = ({ setResult }) => {
  ...
  const fetchData = (value) => {
    ...
    if (selectedFilter === "location") {
      ...
      filteredResults = filteredResults.filter((item) =>
        new Date(item.eventDate).toLocaleDateString().includes(lowercasedValue)
      );
    } else {
      ...
      {selectedFilter && (
        <div className="filters">
          <button
            ...
            className={`filterBtn ${selectedFilter === "eventType" ? "active" : ""}`}
          >
            Vrsta događaja
          </button>
          <button
            onClick={() => handleSelectFilter("location")}
            className={`filterBtn ${selectedFilter === "location" ? "active" : ""}`}
          >
            Lokacija
          </button>
          <button
            onClick={() => handleSelectFilter("performer")}
            className={`filterBtn ${selectedFilter === "performer" ? "active" : ""}`}
          >
            Izvođač
          </button>
          <button
            onClick={() => handleSelectFilter("date")}
            className={`filterBtn ${selectedFilter === "date" ? "active" : ""}`}
          >
            Datum
          </button>
        </div>
